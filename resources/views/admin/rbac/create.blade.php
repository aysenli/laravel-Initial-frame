		...
		<form method="post" action="{{ route('admin.rbac.roles.store') }}">
			<table class="form_table" cellpadding="0" cellspacing="0" id="tab-1">
				...
				<tr>
					...
					<td>
						<input type="text" name="display_name" value="" class="normal"   />
			...
			<table class="form_table" cellpadding="0" cellspacing="0" id="tab-3">
				...
				<tr>
					<th style="font-size: 14px; text-align: center;">
						<label class="attr">
							<input type="checkbox" name="navigation_id[]" data-id="{{ $element->id }}" value="{{ $element->id }}"/>
							{{ $element->name }}
						</label>
					</th>					
					<td>											
				...
				<tr>
					<th>
						<label class="attr">
							<input type="checkbox" name="navigation_id[]" data-pid="{{ $el->pid }}" data-id="{{ $el->id }}" value="{{ $el->id }}"/>
							{{ $el->name }}
						</label>
					</th>						
					<td>	
						...
						<label class="attr">
							<input type="checkbox" data-pid="{{ $vo->pid }}" name="navigation_id[]" value="{{ $vo->id }}"/>
							{{ $vo->name }}
						</label>
						@endforeach				
						@endif	
